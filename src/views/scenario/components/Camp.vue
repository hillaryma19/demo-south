      ...
      <div class="list">
        <div class="list-1" v-show="listType == 1">
          ...
          <ul class="metrix-ul-vertical flex-column">
            <li
              v-for="(item, pindex) in formatData"
              :key="item.id"
              class="flex-start"
            >
              <div
              ...
              <div class="metrix-relationship0 metrix-relationship flex-row">
                <div v-for="(child, index) in item.children" :key="child.id">
                  ...
                  <el-select
                    v-else
                    size="small"
                    v-model="child[`val${pindex}${index}`]"
                    placeholder="请选择"
                    @change="handleRelationChange($event, pindex, index)"
                  >
                    <el-option
...
export default {
  ...
  created() {
    this.formatListData();
    this.formatData = JSON.parse(JSON.stringify(this.dynamicTags));
  ...
  methods: {
    ...
    */
    ...
    handleRelationChange(val, pindex, index) {
      this.formatData.forEach((item, i) => {
        if (i == pindex) {
